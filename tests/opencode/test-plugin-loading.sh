...
# Test 3: Check using-man skill exists (critical for bootstrap)
echo "Test 3: Checking using-man skill (required for bootstrap)..."
if [ -f "$SUPERPOWERS_SKILLS_DIR/using-man/SKILL.md" ]; then
    echo "  [PASS] using-man skill exists"
else
    echo "  [FAIL] using-man skill not found (required for bootstrap)"
    exit 1
fi
