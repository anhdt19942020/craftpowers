...
> **For agentic workers:** REQUIRED SUB-SKILL: Use man:executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "man:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip man: prefix if present
    const forceSuperpowers = skillName.startsWith('man:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^man:/, '') : skillName;

    // Try personal skills first (unless explicitly man:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "man:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'man:';
...
**Skills naming:**
- Superpowers skills: \`man:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'man:using-man' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "man:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill man:brainstorming | head -20`
